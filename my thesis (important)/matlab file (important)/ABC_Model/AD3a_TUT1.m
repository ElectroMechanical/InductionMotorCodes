...
%               for potokoceplenii
% Used program function VpkYUT1.m
%=========================================
clc, clf,
...
Mc=17.45;  % 17.45 Moment nagruzki, [N*M]; Mnom =17.45 {N*M]
%===========================
w1=2*pi*f;
Un=Unom*sqrt(2);
...
[t,y]=ode45(@VpkTUT1,tspan,y0);%- TUT TORMOZENIE
n=max(size(t));
m=zeros(1,n);
i1=zeros(n,2);
i2=zeros(n,2);
...
i2(:,2)=a2.*y(:,4)-a3.*y(:,2);
%===================================
%Callculation tpusk, twok and ttorm
Si=0;
for i=1:n
    if t(i)<=0.3
    dd=t(i+1)-t(i);
    Si=Si+3*(i1(i,1)^2+i1((i+1),1)^2)/2*rs*dd; 
else
    tw=t(i);   k=i;
    break
     end
...
%==================================
[a,j]=min(y(:,5));
IntPelp=3.*trapz(t(1:kp),i1(1:kp,1).^2).*rs;% Start losses (pusk poteri)
IntPelw=3.*trapz(t(kp:k),i1(kp:k,1).^2).*rs;% Work losses (poteri v ust. rezime)
IntPel=Si;% Start and work losses (pusk+work poteri)
IntPelt=3*trapz(t(k:j,1),i1(k:j,1).^2).*(3.*rs);% Rreak losses (tormoz poteri)
disp(['   ','LOSSES OF START, WORK AND BREAKE']);
disp(['    ','Tpusk','     ','Twork','    ',' Ttorm','[c]']);
tk=t(j);
disp([tp,tw,tk]);
disp(['  ','Pcupusk','   ','Pcuwork','  ',' Pcutorm','[Wt]']);
disp([IntPelp,IntPelw,IntPelt]);
%=================================
figure(1);
subplot(2,1,1)
H1=plot(t(:),m(:),'r'); grid,
