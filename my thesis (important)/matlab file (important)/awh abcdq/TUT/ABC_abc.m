 ...
 clc;
 clf;
% File - ABC_abc.m: Callculation  Diff. Equestion of AD
...
%iar(:)=A(4,1).*y(:,1)+A(4,2).*y(:,2)+A(4,3).*y(:,3)+A(4,4).*y(:,4)+A(4,5).*y(:,5)+A(1,6).*y(:,6);
%ibr(:)=A(5,1).*y(:,1)+A(5,2).*y(:,2)+A(5,3).*y(:,3)+A(5,4).*y(:,4)+A(5,5).*y(:,5)+A(1,6).*y(:,6);
me(:)=Km.*(y(:,1).*(ib(:)-ic(:))+y(:,2).*(ic(:)-ia(:))+y(:,3).*(ia(:)-ib(:)));
Mmax=max(me(:));
Km=Mmax/Mnom;
Imax=max(ia(:));
Ki=Imax/In;
disp([' ','Km=Mud/Mnom','  ','Ki=Iud/Inom']);
disp([Km,Ki]);
%============================================
figure(1),
subplot(4,4,[1,2,5,6]);
...
%=========================================
figure(2),
H3=plot(t(:),y(:,1),'k');grid;
set(H3,'LineWidth',2);
hx3=XLABEL('t, [c]');
set(hx3,'FontSize',10,'FontWeight','bold');
hy3=YLABEL('Psi_{A}, [Wb]');
