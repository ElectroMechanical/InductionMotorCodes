...
clc;
clf;
% SINCHRON GENERATOR WITH PM - KC37A- SmCo
% Radial construction
% echo on
%        Calculation sinchron generator with Permanent Magnet;
%        =====================================================     
disp('  Student of grup EL-11-2000:  SEMECHKIN A.A.')
...
disp(' Magnit SmCo')
% ===========================================
br=0.82;     % Br - Remanence induction [T}, 
hc=560e3; % Hc - Koerchitivnaia forse [A/M],
...
%==================================
% Vvod  new parametrs generators
%==================================
disp('    After vvoda New value - push the button <<ENTER>>');
k=1;
 while k<10
    k=menu('What will vary ',...
    sprintf('Power- Snom=%g kWt ',sn),...
    sprintf('Voltage- Unom=%g V ',un),...
    sprintf('Frequency- f1=%g Hz ',f1),...
    sprintf('Number par of poles- p=%g o.e. ',p),...
    sprintf('Coefficient power- cos(fi) =%g o.e ',cos_fi),...
    sprintf('Coefficient- u=%g o.e ',u),...
    sprintf('Remanence indution- Br=%g T ',br),...
    sprintf('Coersitive forse- Hc=%g A/M ',hc),...
    sprintf('Coefficient- gammma=%g o.e ',gamma),...
        'Nothinng not will vary');
     if k==1,
sn=input([sprintf(' Initial value Snom=%g',sn)...
        '  kWt, New value Sn= ']);
     elseif k==2,
un=input([sprintf(' Initial value Unom=%g',un)...
        ' V, New value Un= ']);
     elseif k==3,
f1=input([sprintf(' Initial value f1=%g',f1)...
        '  Hz, New value f1= ']);
      elseif k==4,
p=input([sprintf(' Initial value p=%g',p)...
        '  o.e, New value p= ']);
      elseif k==5
cos_fi=input([sprintf(' Initial value cos(fi)=%g',cos_fi)...
        '  o.e., New value cos(fi)= ']);
      elseif k==6,
u=input([sprintf(' Initial value u=%g',u)...
        '  o.e, New value u= ']);
      elseif k==7,
br=input([sprintf(' Initial value Br=%g',br)...
        '  T, New value Br= ']);
     elseif k==8,
hc=input([sprintf(' Initial value Hc=%g',hc)...
        '  A/M, New value Hc= ']);
      elseif k==9,
gamma=input([sprintf(' Initial value gamma=%g',gamma)...
        '  o.e, New value gamma= ']);
         end
  end 
